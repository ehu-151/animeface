...
for file in files:
    ...
    for (x, y, w, h) in faces:
        ...
        # これから検出する範囲を囲む
        cv2.rectangle(img, (x - range, y - range), (range + x + w, range + y + h), (0, 0, 255), 2)
        # 顔を検知
        faces = ren_cascade.detectMultiScale(gray[y - range:range + y + h, x - range:range + x + w])
        for (rx, ry, rw, rh) in faces:
            # 検知した顔を矩形で囲む
            cv2.rectangle(img, ((x-range)+rx, (y-range)+ry), ((x-range)+rx+ rw, (y-range)+ry + rh), (0, 255, 0), 2)
